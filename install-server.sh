...
die() {
  echo "[nginx-manager-bootstrap] 错误：$*" >&2
  exit 1
}
...
WORK_DIR="$(mktemp -d /tmp/nginx-manager-bootstrap.XXXXXX)"
trap 'rm -rf -- "${WORK_DIR}"' EXIT
umask 077

ARCHIVE="${WORK_DIR}/source.tar.gz"
DOWNLOAD_URL="https://codeload.github.com/${REPOSITORY}/tar.gz/${REF}"
echo "[nginx-manager-bootstrap] 正在从 GitHub 下载 ${REPOSITORY}@${REF}"
...
if [[ -n "${ARCHIVE_SHA256}" ]]; then
  ...
  echo "[nginx-manager-bootstrap] 警告：正在使用可变 main；生产环境请固定 commit 并启用校验" >&2
fi
...
INSTALLER="${WORK_DIR}/${TOP_LEVEL}/deploy/install-server.sh"
[[ -f "${INSTALLER}" ]] || die "归档中缺少 deploy/install-server.sh"
bash "${INSTALLER}" "$@"
