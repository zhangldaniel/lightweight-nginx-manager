...
die() {
  echo "[nginx-manager-uninstall] 错误：$*" >&2
  exit 1
}
...
WORK_DIR="$(mktemp -d /tmp/nginx-manager-uninstall.XXXXXX)"
...
UNINSTALLER="${WORK_DIR}/${TOP_LEVEL}/deploy/uninstall-server.sh"
[[ -f "${UNINSTALLER}" ]] || die "归档中缺少 deploy/uninstall-server.sh"
bash "${UNINSTALLER}" "$@"
