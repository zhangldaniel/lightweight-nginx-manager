...
die() {
  echo "[nginx-manager-container-bootstrap] 错误：$*" >&2
  exit 1
}
...
WORK_DIR="$(mktemp -d /tmp/nginx-manager-container-bootstrap.XXXXXX)"
...
if [[ -n "${ARCHIVE_SHA256}" ]]; then
  ...
  echo "[nginx-manager-container-bootstrap] 警告：正在使用可变 main；生产迁移建议固定 commit" >&2
fi
...
bash "${WORK_DIR}/${TOP_LEVEL}/deploy/migrate-server-to-container.sh" "$@"
